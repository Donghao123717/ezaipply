...
export function TestingReview() {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${base}/api/upload/testing?user_id=${userId}`)
  ...
  const handleDelete = async (filename: string) => {
    ...
    try {
      const base = process.env.NEXT_PUBLIC_BACKEND_URL || '/api/backend'
      const res = await fetch(`${base}/api/upload/testing/${encodeURIComponent(filename)}?user_id=${userId}`, {
        method: 'DELETE',
      })
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h2 className="text-2xl font-semibold tracking-tight">Testing Documents</h2>
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Review and manage your uploaded testing documents (SAT, ACT, AP scores, etc.)
        </p>
      </div>
      ...
      {!error && files.length === 0 && (
        <div className="rounded-lg border border-dashed border-muted-foreground/40 p-12 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Upload your testing documents to get started
          </p>
          <Link href="/starter/testing/upload">
            <Button>
