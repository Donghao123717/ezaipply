...
export function EducationReview() {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${base}/api/upload/education?user_id=${userId}`)
  ...
  const handleDelete = async (filename: string) => {
    ...
    try {
      const base = process.env.NEXT_PUBLIC_BACKEND_URL || '/api/backend'
      const res = await fetch(`${base}/api/upload/education/${encodeURIComponent(filename)}?user_id=${userId}`, {
        method: 'DELETE',
      })
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <h2 className="text-2xl font-semibold tracking-tight">Education Documents</h2>
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Review and manage your uploaded education documents (transcripts, diplomas, etc.)
        </p>
      </div>
      ...
      {!error && files.length === 0 && (
        <div className="rounded-lg border border-dashed border-muted-foreground/40 p-12 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Upload your education documents to get started
          </p>
          <Link href="/starter/education/upload">
            <Button>
      ...
      {files.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link href="/starter/education/upload" className="flex-1">
