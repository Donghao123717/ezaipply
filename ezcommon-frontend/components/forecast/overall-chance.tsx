  ...
  return (
    <div className="rounded-2xl border bg-card p-6 mb-6">
      <div className="grid sm:grid-cols-[auto_1fr] gap-8 items-start">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-1">{t('forecast.overall.chanceOfOffer')}</p>
          <p className="font-display text-5xl font-semibold text-primary">
            {atLeastOne}
            <span className="text-2xl align-top">%</span>
          </p>
          <p className="text-xs text-muted-foreground mt-2">{t('forecast.overall.basedOn').replace('{count}', String(schools.length))}</p>
          <div className="flex flex-wrap gap-1.5 mt-3">
            <span className="rounded-full border px-2 py-0.5 text-xs">{t(CATEGORY_LABEL_KEY.reach)} · {reach}</span>
            <span className="rounded-full border px-2 py-0.5 text-xs">{t(CATEGORY_LABEL_KEY.target)} · {target}</span>
            <span className="rounded-full border px-2 py-0.5 text-xs">{t(CATEGORY_LABEL_KEY.safety)} · {safety}</span>
            {lowConfidence && (
        ...
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">{t('forecast.overall.portfolioTrend')}</p>
          <div className="h-24 rounded-lg bg-muted/50 flex items-center justify-center relative">
            ...
            <div className="relative flex flex-col items-center">
              <span className="rounded-md bg-primary text-primary-foreground text-xs px-2 py-1 mb-1 whitespace-nowrap">
                {t('forecast.overall.nowLabel').replace('{percent}', String(atLeastOne))}
              </span>
