...
export function ProfileBuilder({ userId, defaultFirstName, defaultLastName }: { userId: string; defaultFirstName: string; defaultLastName: string }) {
  ...
  function applySuggestions(suggestions: { section: string; field: string; value: string; item?: number; nestedKey?: string }[]) {
    setData((prev) => {
      let next = prev

      // Flat (simple-section) fields merge directly into the section's data object.
      ...
      // Repeatable fields (top-level repeatable sections, or nested repeatables inside a
      // simple section) are grouped by (section, nestedKey, item) into new records - one
      // new list item per distinct record the AI found, rather than merged into a single object.
      ...
      for (const s of suggestions) {
        ...
        // Nested-repeatable field keys are qualified as "<nestedKey>.<field>" (see
        // buildFieldSchema) to avoid colliding with a same-named flat field - strip
        // that prefix back off so the record object gets the real field key.
      ...
      for (const { section, nestedKey, fields } of groups.values()) {
        if (nestedKey) {
          ...
          next = {
            ...next,
            [section]: { ...sectionData, [nestedKey]: [...nestedList(sectionData, nestedKey), fields] },
          }
        } else {
          next = { ...next, [section]: [...(((next[section] as RepeatableData) || [])), fields] }
        }
      }
