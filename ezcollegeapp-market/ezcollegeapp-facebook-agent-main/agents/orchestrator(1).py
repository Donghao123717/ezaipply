"""
Orchestrator: central coordinator for the Facebook Playwright automation flow.
"""
...
class Orchestrator:
    ...
    def post_manual(self, media_path: str, caption: str) -> str:
        return self.post_agent.post_manual(media_path=media_path, caption=caption)

    def post_llm(self, media_path: str, post_format: str = "story", extra_context: str = "") -> str:
        return self.post_agent.post_llm(
            media_path=media_path, post_format=post_format, extra_context=extra_context
        )
    ...
    def dm_manual(self, target: str, text: str) -> str:
        return self.dm_agent.dm_manual(target=target, text=text)

    def dm_llm(self, target: str, materials: str, extra_context: str = "") -> str:
        return self.dm_agent.dm_llm(target=target, materials=materials, extra_context=extra_context)
    ...
    def reply_manual(self, post_url: str, manual_replies: dict[str, str]) -> int:
        return self.reply_agent.reply_manual(post_url=post_url, manual_replies=manual_replies)

    def reply_llm(self, post_url: str, extra_context: str = "") -> int:
        return self.reply_agent.reply_llm(post_url=post_url, extra_context=extra_context)
    ...
    def follow_batch(self, page_ids: list[str]) -> dict[str, int]:
        return self.follow_agent.follow_batch(page_ids=page_ids)
    ...
    def run_job(self, job_path: str, mode_override: str = "") -> dict[str, Any]:
        ...
        if post_cfg.get("enabled", False):
            if mode == "manual":
                caption = self.post_manual(
                    media_path=post_cfg["media"], caption=post_cfg["caption"]
                )
            else:
                caption = self.post_llm(
                    media_path=post_cfg["media"],
        ...
        if dm_cfg.get("enabled", False):
            ...
            for rec in dm_cfg.get("recipients", []):
                target = rec["target"]
                if mode == "manual":
                    text = self.dm_manual(target=target, text=rec["text"])
                else:
                    text = self.dm_llm(
                        target=target,
            ...
            summary["results"]["dm"] = {"ok": True, "sent": sent}

        comment_cfg = job.get("comment", {})
        if comment_cfg.get("enabled", False):
            if mode == "manual":
                text = self.comment_manual(
                    post_url=comment_cfg["post_url"], text=comment_cfg["text"]
                )
            else:
        ...
        if reply_cfg.get("enabled", False):
            post_url = reply_cfg["post_url"]
            if mode == "manual":
                manual_replies = {
                    item["comment_id"]: item["text"]
                    for item in reply_cfg.get("manual_replies", [])
                }
                count = self.reply_manual(post_url=post_url, manual_replies=manual_replies)
            else:
        ...
        if follow_cfg.get("enabled", False):
            page_ids = self._load_follow_accounts(follow_cfg)
            result = self.follow_batch(page_ids)
            summary["results"]["follow"] = {"ok": True, **result}
