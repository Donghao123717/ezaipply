...
def load_hashtag_guide(hashtag: str) -> str:
    """Load a hashtag-specific guide if available, else return empty string."""
    path = _PROMPTS_DIR / f"hashtag_{hashtag.lstrip('#')}.md"
    if path.exists():
        return path.read_text()
    return ""
