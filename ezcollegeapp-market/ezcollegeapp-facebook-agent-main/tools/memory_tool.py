...
_DEFAULT_STATE = {
    ...
    "replied_comment_ids": [],
    "followed_usernames": [],
    "friend_requested_usernames": [],
    "daily_comment_count": 0,
    "daily_post_count": 0,
    "daily_follow_count": 0,
    "daily_friend_request_count": 0,
    "last_reset_date": "",
}
...
class MemoryTool:
    ...
    def has_friend_requested(self, username: str) -> bool:
        """Return True if we have already sent a friend request to this username."""
        state = self.load_state()
        return username in state.get("friend_requested_usernames", [])

    def mark_friend_requested(self, username: str) -> None:
        """Record that we sent a friend request and increment daily counter."""
        state = self.load_state()
        if username not in state.setdefault("friend_requested_usernames", []):
            state["friend_requested_usernames"].append(username)
        state["daily_friend_request_count"] = state.get("daily_friend_request_count", 0) + 1
        self.save_state(state)

    def daily_friend_request_count(self) -> int:
        return self.load_state().get("daily_friend_request_count", 0)
    ...
    def _maybe_reset_daily_counters(self, state: dict) -> dict:
        ...
        if state.get("last_reset_date") != today:
            ...
            state["daily_friend_request_count"] = 0
